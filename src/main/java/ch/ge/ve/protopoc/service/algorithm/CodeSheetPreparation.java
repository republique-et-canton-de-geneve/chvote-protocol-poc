...
import ch.ge.ve.protopoc.service.model.Voter;
...
public class CodeSheetPreparation {
     ...
     */
    public List<CodeSheet> getSheets(ElectionSet electionSet, List<List<ElectionPreparation.SecretVoterData>> codeSheetData) {
        ...
        for (int i = 0; i < electionSet.getVoters().size(); i++) {
            ...
            final int local_i = i;
            List<ElectionPreparation.SecretVoterData> secretVoterDataList = codeSheetData.stream()
     ...
     */
    private byte[] computeX_i(List<ElectionPreparation.SecretVoterData> secretVoterDataList) {
        BigInteger sum_x_ij = IntStream.range(0, s).mapToObj(secretVoterDataList::get)
                .map(ElectionPreparation.SecretVoterData::getX)
     ...
     */
    private byte[] computeY_i(List<ElectionPreparation.SecretVoterData> secretVoterDataList) {
        BigInteger sum_y_ij = IntStream.range(0, s).mapToObj(secretVoterDataList::get)
                .map(ElectionPreparation.SecretVoterData::getY)
                .reduce(BigInteger::add).orElseGet(() -> BigInteger.ZERO);
     ...
     */
    private byte[] computeF_i(List<ElectionPreparation.SecretVoterData> secretVoterDataList) {
        return IntStream.range(0, s).mapToObj(secretVoterDataList::get)
                .map(ElectionPreparation.SecretVoterData::getF)
     ...
     */
    private byte[][] computeRC_i(ElectionSet electionSet, List<ElectionPreparation.SecretVoterData> secretVoterDataList) {
