...
public class ElectionPreparation {
     ...
     */
    public List<Polynomial.Point> getPublicCredentials(List<List<Polynomial.Point>> D_circ) {
        ...
        return IntStream.range(0, D_circ.get(0).size()).parallel().mapToObj(i -> {
            ...
            for (List<Polynomial.Point> d_circ_j : D_circ) {
                Polynomial.Point voterData_ij = d_circ_j.get(i);
            ...
            return new Polynomial.Point(x_circ_i, y_circ_i);
        }).collect(Collectors.toList());
    }
