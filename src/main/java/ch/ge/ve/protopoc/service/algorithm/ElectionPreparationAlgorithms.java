...
public class ElectionPreparationAlgorithms {
    ...
    /**
     * Algorithm 7.9: GenElectorateData
     *
    ...
    public SecretVoterData genSecretVoterData(List<Point> points) {
        ...
        byte[] F = ByteArrayUtils.truncate(hash.recHash_L(points.toArray()), publicParameters.getL_f() / 8);
        byte[][] rc = new byte[points.size()][];

        for (int i = 0; i < points.size(); i++) {
            rc[i] = ByteArrayUtils.truncate(hash.recHash_L(points.get(i)), publicParameters.getL_r() / 8);
        }
    ...
    public Point getPublicVoterData(BigInteger x, BigInteger y, List<BigInteger> yList) {
        BigInteger local_y = y.add(conversion.toInteger(hash.recHash_L(yList.toArray()))).mod(identificationGroup.getQ_circ());
    ...
    /**
     * Algorithm 7.15: GetPublicCredentials
     *
